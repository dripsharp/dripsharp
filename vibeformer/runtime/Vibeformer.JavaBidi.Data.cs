    ...
    internal sealed class BidiData
    {
        ...
        public void Append(ReadOnlySpan<char> text)
        {
            ...
            foreach (var rune in text.EnumerateRunes())
            {
                ...
                var pbt = BidiUnicodeData.GetBiDiPairedBracketType(codepoint);
                ...
                if (pbt == BidiPairedBracketType.Open)
                {
                    ...
                    var paired = BidiUnicodeData.GetBiDiPairedBracket(codepoint);
                    _pairedBracketValues[i] = (int)BidiUnicodeData.GetCanonicalType(paired);

                    HasBrackets = true;
                }
                else if (pbt == BidiPairedBracketType.Close)
                {
                    _pairedBracketValues[i] = (int)BidiUnicodeData.GetCanonicalType(codepoint);
