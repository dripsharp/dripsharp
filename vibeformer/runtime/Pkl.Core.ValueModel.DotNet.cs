...
internal static class PklUris
{
    ...
    public static Uri Resolve(Uri baseUri, Uri newUri)
    {
        ...
        string baseText = Vibeformer.Runtime.JavaCompat.UriToString(baseUri);
        if (Vibeformer.Runtime.JavaCompat.UriUsesSingleSlashFileSyntax(baseUri) ||
            (baseText.StartsWith("file:/", StringComparison.OrdinalIgnoreCase) &&
                !baseText.StartsWith("file://", StringComparison.OrdinalIgnoreCase)))
        {
            string basePath = baseText["file:".Length..];
            var carrier = new Uri("https://vibeformer.invalid" + basePath);
            Uri resolved = new(carrier, newUri);
            return Vibeformer.Runtime.JavaCompat.CreateUri(
    ...
    {
        ArgumentNullException.ThrowIfNull(uri);
        return Vibeformer.Runtime.JavaCompat.UriToString(uri);
    }
    ...
    public static Uri Relativize(Uri uri, Uri baseUri)
    {
        ...
        if (!uri.IsAbsoluteUri || !baseUri.IsAbsoluteUri ||
            Vibeformer.Runtime.JavaCompat.UriIsOpaque(uri) ||
            Vibeformer.Runtime.JavaCompat.UriIsOpaque(baseUri) ||
            !string.Equals(uri.Scheme, baseUri.Scheme, StringComparison.Ordinal) ||
            !string.Equals(
                Vibeformer.Runtime.JavaCompat.UriRawAuthority(uri),
                Vibeformer.Runtime.JavaCompat.UriRawAuthority(baseUri),
                StringComparison.Ordinal))
            return uri;

        string uriPath = Vibeformer.Runtime.JavaCompat.UriPath(uri) ?? "";
        string basePath = Vibeformer.Runtime.JavaCompat.UriPath(baseUri) ?? "";
