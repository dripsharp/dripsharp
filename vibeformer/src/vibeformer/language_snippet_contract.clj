(ns vibeformer.language-snippet-contract
...
(def ^:private manifest-magic "VIBEFORMER_LANGUAGE_SNIPPET_CONTRACT_V1")
...
(defn verify-contract!
  ...
  ([{:keys [workspace-root manifest run-command!]
   ...
   (let [root (paths/absolute (or workspace-root (paths/workspace-root)))
         ...
         oracle-source (paths/resolve-path root "vibeformer" "validation"
                                           "language-snippet-contract"
         ...
         (fn [output]
           (run-command!
            {:command ["./gradlew" "-I" (str init-script)
                       ":pkl-core:vibeformerLanguageSnippetOracle" "--console=plain"
                       (str "-Pvibeformer.oracleClasses=" oracle-classes)
                       (str "-Pvibeformer.manifest=" (paths/absolute manifest))
                       (str "-Pvibeformer.output=" output)]
             :directory upstream}))]
