...
(def ^:private manifest-header "VIBEFORMER_MAVEN_REACTOR_V1")
(def ^:private extension-source
  (paths/path "vibeformer/maven/src/vibeformer/maven/DiscoveryEventSpy.java"))
(def ^:private extension-components
  (paths/path "vibeformer/maven/resources/META-INF/plexus/components.xml"))
...
(defn- cache-root
  ...
  (let [configured (System/getenv "XDG_CACHE_HOME")
    ...
    (paths/absolute (paths/resolve-path base "vibeformer" "maven"))))
...
(defn- build-extension!
  ...
  (let [root (paths/absolute (or runner-cache (cache-root)))
        ...
        output (paths/resolve-path
                extension-root
                (str "vibeformer-maven-discovery-" input-hash ".jar"))]
...
(defn discover-reactor!
  ...
  (let [workspace-root (paths/absolute workspace-root)
        ...
        (cond-> [(str launcher)
                 ...
                 (str "-Dvibeformer.discovery.manifest=" manifest)
