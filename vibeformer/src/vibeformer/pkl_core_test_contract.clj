(ns vibeformer.pkl-core-test-contract
...
(def ^:private manifest-magic "VIBEFORMER_PKL_CORE_TEST_CONTRACT_V1")
...
(defn- layout
  ...
  (let [root (paths/absolute workspace-root)
    ...
    {:root root
     ...
     :manifest (paths/resolve-path root "vibeformer" "validation"
...
(defn- evidence-metadata
  []
  [["evidence-parser" "vibeformer/validation/differential/UpstreamOracle.java"]
   ["evidence-language"
    "vibeformer/validation/language-snippet-contract/LanguageSnippetContract.tsv"]
   ["evidence-core" "vibeformer/validation/differential/CoreUpstreamOracle.java"]
   ["evidence-loading" "vibeformer/validation/loading-contract/ContractEvidence.tsv"]
   ["evidence-public-api" "vibeformer/validation/public-api-contract/BehaviorContract.tsv"]
   ["evidence-schema-codegen"
    "vibeformer/validation/schema-codegen/ContractEvidence.tsv"]
   ["evidence-binding" "vibeformer/validation/schema-codegen/ContractEvidence.tsv"]])
     ...
     (concat
      [["source-repository" "https://github.com/apple/pkl.git"]
       ...
       ["listener-source-sha256"
        (source-hash
         "vibeformer/validation/pkl-core-test-contract/PklCoreTestDiscoveryListener.java")]
       ["listener-service-sha256"
        (source-hash
         "vibeformer/validation/pkl-core-test-contract/META-INF/services/org.junit.platform.launcher.TestExecutionListener")]
       ["gradle-init-sha256"
        (source-hash "vibeformer/gradle/pkl-core-test-contract.gradle")]]
      (evidence-metadata)))))
...
(defn verify-contract!
  ...
  ([{:keys [workspace-root manifest run-command!]
   ...
   (let [root (paths/absolute (or workspace-root (paths/workspace-root)))
     ...
     (run-command!
      {:command ["env" "VIBEFORMER_WORKERS=22" "GRADLE_OPTS=-Xmx28g"
                 "./gradlew" "-I" (str (:init-script layout))
                 ":pkl-core:test" "--console=plain"
                 (str "-Pvibeformer.contractDir=" (:contract-dir layout))
                 (str "-Pvibeformer.discoveryOutput=" raw)]
