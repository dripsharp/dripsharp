...
(def ^:private observation-header "VIBEFORMER_RAWHTTP_OBSERVATIONS_V1")
(def ^:private provenance-header "VIBEFORMER_RAWHTTP_PACKAGE_PROVENANCE_V1")
...
(def ^:private profile-file "vibeformer/config/rawhttp-core.edn")
...
(defn- read-contract! [^Path file]
  (let [[header & lines] (str/split-lines (Files/readString file StandardCharsets/UTF_8))
        ...
        duplicates (->> grouped (keep (fn [[key values]] (when-not (= 1 (count values)) key)))
                        sort vec)]
    (when-not (= "VIBEFORMER_JAVA_LIBRARY_CONTRACT_V1" header)
...
(defn audit-selected-surface!
  ...
  (let [generation (get-in package-proof [:verification :generation])
        ...
        (->> source-files
             ...
             vec)
        compiler (paths/resolve-path root "vibeformer" "validation"
    ...
    (bounded-run! {:command ["dotnet" "run" "--project" (str compiler)
                   ...
                   :directory root})
    (let [audit-rows
          (parse-tsv! audit-file "# VIBEFORMER_ACCESSIBLE_BODY_AUDIT_V1"
          ...
          reviews
          (parse-tsv! review-file "VIBEFORMER_RAWHTTP_BODY_REVIEW_V1"
...
(defn verify-package-equivalence!
  ...
  ([{:keys [workspace-root run-command! package-fn]
   ...
   (let [root (paths/absolute (or workspace-root (paths/workspace-root)))
         contract-file (paths/resolve-path root "vibeformer" "validation"
                                           "rawhttp-core" "ProjectContract.tsv")
         contract (read-contract! contract-file)
         java-proof
         (run-command! {:command ["sh" (str (paths/resolve-path root "vibeformer"
                                                                "validation" "rawhttp-core"
                                                                "verify.sh"))]
