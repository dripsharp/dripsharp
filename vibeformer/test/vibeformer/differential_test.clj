...
(deftest loading-contract-is-source-backed-executable-and-retains-pending-scope
  (let [root (paths/workspace-root)
    ...
    (is (= 59 (:families summary)))
    (is (= 59 (:existing-evidence summary)))
    (is (zero? (:pending-in-scope summary)))
    (is (= 52 (:jvm-shared-families summary)))
    (is (= 7 (:dotnet-adaptation-families summary)))
    (is (= 18 (:jvm-shared-observations summary)))
    (is (= 6 (:dotnet-adaptation-observations summary)))
