(ns dripsharp.pkl.brine-xunit
  ...
  (:require [clojure.edn :as edn]
            ...
            [dripsharp.pkl.parser-test-contract :as parser-contract]
...
(def ^:private required-authored-sources
  #{"src/dripsharp/consumer_tests.clj"
    ...
    "validation/pkl-parser-suite/PklParserPackageSuiteRunner.cs"})
...
(defn- render-boundary-report
  [rows language-count core-count parser-count]
  (let [counts (frequencies (map #(get % "class") rows))]
    (str
     ...
     "524 apply on Windows), plus " parser-count
     " complete pkl-parser adapted rows.\n\n"
...
(defn emit!
  ...
  (let [root (paths/workspace-root (:target-directory target-contract))
        ...
        parser-manifest (paths/resolve-path
                         root "validation/pkl-parser-test-contract"
        ...
        parser (parser-contract/validate-manifest! root parser-manifest)
        ...
        parser-runner
        (paths/resolve-path tests-root "DripSharp.Brine.ParserTestRunner")
        ...
        parser-contract-output
        (paths/resolve-path contracts "PklParserTestContract.tsv")
        expected-output
    ...
    (doseq [[source output source-path transformation]
            [[language-manifest language-contract-output
             ...
             [core-manifest core-contract-output
              ...
              "validated-normalized-contract-copy-v1"]
             [parser-manifest parser-contract-output
              "validation/pkl-parser-test-contract/PklParserTestContract.tsv"
              "validated-normalized-contract-copy-v1"]]]
      (copy-file! source output)
      (swap! rows conj
    ...
    (doseq [[source-path runner project-name embedded?]
            [["validation/language-snippet-runner/LanguageSnippetPackageRunner.cs"
             ...
             ["validation/pkl-parser-suite/PklParserPackageSuiteRunner.cs"
              parser-runner "DripSharp.Brine.ParserTestRunner" false]]]
      ...
      (let [proof (verify-provenance! tests-root)]
        {:language-cases (count (:cases language))
         ...
         (count (filter #(= "complete-pkl-core-runner"
                        ...
                        (:cases core)))
         :parser-junit-cases
         (count (distinct (map :junit-unique-id (:cases parser))))
         :parser-adapted-cases (count (:cases parser))
         :provenance proof}))))
