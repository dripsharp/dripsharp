(ns dripsharp.concurrency
  "One bounded executor policy for DripSharp CPU and subprocess work."
  (:import [java.util.concurrent Callable ExecutionException ExecutorService Executors
            ThreadFactory TimeUnit]
           [java.util.concurrent.atomic AtomicInteger]))

(def ^:private worker-property "dripsharp.workers")
(def ^:private worker-environment "DRIPSHARP_WORKERS")
  ...
  (when value
    (let [parsed (try
                   ...
                   (catch NumberFormatException error
                     (throw (ex-info "DripSharp worker count must be a positive integer"
...
(defn call-with-executor
  ...
  ([{:keys [worker-count thread-prefix]
     :or {thread-prefix "dripsharp-worker"}}
    f]
   (if *executor*
     (f)
                             ...
                             (fn [[index input]]
                               (try
                                 ...
                                 (catch Throwable error
                                   (throw (ex-info
                                           (str "Concurrent DripSharp phase failed: " (name phase)
                                                ": " (.getMessage error))
