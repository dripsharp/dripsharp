(ns dripsharp.pdfcube.pdfbox-document-lifecycle-differential
  "Pinned PDFBox 3.0.8 versus package-only PdfCube.PdfBox document lifecycle proof."
...
(def required-trace-families
  #{"catalog" "content-error" "content-stream" "document-info"
    "document-lifecycle" "document-version" "import" "loader-error"
    "package-reopen" "page-inheritance" "page-tree-error"
    "page-tree-malformed" "page-tree-mutation" "resource-cache"
    "resource-ownership" "scratch-storage" "stream-lifetime"})
...
(defn- fail! [message data]
  (throw
   (ex-info
    message
    (assoc data
           :kind :pdfcube-pdfbox-document-lifecycle-differential-failed))))
...
(defn trace-summary
  "Validates a normalized document-lifecycle trace and returns its coverage."
  [trace]
  (let [trace (paths/path trace)
         ...
         (fn [index line]
           (let [fields (str/split line #"\t" -1)]
             (when-not (and (= 3 (count fields))
               ...
               (fail! "Document-lifecycle trace contains a malformed observation"
    ...
    (when-not (seq rows)
      (fail! "Document-lifecycle trace contains no observations"
             {:trace (str trace)}))
    (when (seq duplicates)
      (fail! "Document-lifecycle trace contains duplicate observation identities"
             {:trace (str trace) :duplicates duplicates}))
    (when (seq missing)
      (fail! "Document-lifecycle trace misses required behavior families"
             {:trace (str trace)
              :missing missing
...
(defn- assert-match! [expected actual]
  (let [expected-summary (trace-summary expected)
    ...
    (when-let [mismatch (:mismatch comparison)]
      (fail! "Package-only PdfCube.PdfBox document lifecycle differs from pinned PDFBox 3.0.8"
             {:expected (str expected)
              :actual (str actual)
              :comparison comparison
              :mismatch mismatch}))
    (when-not (= expected-summary actual-summary)
      (fail! "Document-lifecycle trace coverage differs between runtimes"
...
(defn- compile-and-run-oracle!
  ...
  (let [{:keys [release java javac]} (java-tools root generation)
        ...
        oracle-source
        (paths/resolve-path
         root "validation" "pdfcube-pdfbox-document-lifecycle"
         "PdfBoxDocumentLifecycleOracle.java")
        classes
    ...
    (run-command! {:command [(str java) "-classpath" run-classpath
                             "PdfBoxDocumentLifecycleOracle"
                             (str output) (str exchange)]
                   :directory root
                   :timeout-ms 300000})))

(defn- run-package-probe!
  [run-command! ^Path root package-proof ^Path output ^Path exchange mode]
  (let [generation (get-in package-proof [:verification :generation])
        ...
        (paths/resolve-path root "validation"
                            "pdfcube-pdfbox-document-lifecycle" "Program.cs")
        arguments
        (cond-> [(str output) (str exchange)]
          mode (conj mode))]
...
(defn verify!
  ...
  normalized Java/.NET document lifecycle differential validation."
  ([] (verify! {}))
  ([{:keys [workspace-root package-fn run-command!]
   ...
   (let [root (paths/absolute (or workspace-root (paths/workspace-root)))
     ...
     (run-package-probe!
      run-command! root package-proof dotnet-trace exchange "--write-only")
     (compile-and-run-oracle!
      run-command! root generation proof-root java-trace exchange)
     (run-package-probe!
      run-command! root package-proof dotnet-trace exchange nil)
