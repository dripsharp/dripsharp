(ns dripsharp.pdfcube.pdfbox-interchange-differential
  "Pinned reviewed PDFBox baseline versus package-only DripSharp.PdfCarton logical-structure,
  optional-content, FDF, and XFDF proof."
...
(def required-trace-families
  #{"fdf-failure" "fdf-fixture" "fdf-model" "fdf-roundtrip"
    "marked-content" "optional-membership" "optional-model"
    "optional-render" "optional-roundtrip" "structure-attributes"
    "structure-fixture" "structure-model" "structure-parent-tree"
    "structure-roundtrip" "xfdf-annotations" "xfdf-encoding"
    "xfdf-failure" "xfdf-model" "xfdf-namespace" "xfdf-roundtrip"})
...
(defn- fail! [message data]
  (throw
   (ex-info
    message
    (assoc data
           :kind :pdfcube-pdfbox-interchange-differential-failed))))
...
(defn trace-summary
  "Validates a normalized interchange trace and returns its coverage."
  [trace]
  (let [trace (paths/path trace)
         ...
         (fn [index line]
           (let [fields (str/split line #"\t" -1)]
             (when-not (and (= 3 (count fields))
               ...
               (fail! "Interchange trace contains a malformed observation"
    ...
    (when-not (seq rows)
      (fail! "Interchange trace contains no observations"
             {:trace (str trace)}))
    (when (seq duplicates)
      (fail! "Interchange trace contains duplicate observation identities"
             {:trace (str trace) :duplicates duplicates}))
    (when (seq missing)
      (fail! "Interchange trace misses required behavior families"
             {:trace (str trace)
              :missing missing
...
(defn- assert-match! [expected actual]
  (let [expected-summary (trace-summary expected)
    ...
    (when-let [mismatch (:mismatch comparison)]
      (fail!
       "Package-only DripSharp.PdfCarton interchange behavior differs from pinned reviewed PDFBox baseline"
       {:expected (str expected)
        :actual (str actual)
        :comparison comparison
        :mismatch mismatch}))
    (when-not (= expected-summary actual-summary)
      (fail! "Interchange trace coverage differs between runtimes"
...
(defn- compile-and-run-oracle!
  ...
  (let [{:keys [release java javac]} (java-tools root generation)
        ...
        oracle-source
        (paths/resolve-path
         root "validation" "pdfcube-pdfbox-interchange"
         "PdfBoxInterchangeOracle.java")
        classes
...
(defn- run-package-probe!
  ...
  (let [generation (get-in package-proof [:verification :generation])
        ...
        (paths/resolve-path root "validation"
                            "pdfcube-pdfbox-interchange" "Program.cs")
        arguments
...
(defn verify!
  ...
  normalized Java/.NET logical-structure, optional-content, FDF, and XFDF
  validation."
  ([] (verify! {}))
     ...
     (let [comparison (assert-match! java-trace dotnet-trace)
           ...
           {:profile "pdfcube-pdfbox"
            ...
            ["org/apache/pdfbox/pdfparser/withcatalog.fdf"
             ...
             "org/apache/pdfbox/pdmodel/documentinterchange/logicalstructure/PDFBOX-2725-878725.pdf"
       ...
       (println
        "Pinned Java/package DripSharp.PdfCarton interchange differential passed:"
