...
[ "$(sed -n '1p' "$contract")" = "DRIPSHARP_JAVA_LIBRARY_CONTRACT_V1" ] \
...
baseline_file="$workspace_root/$(contract_value baseline-file)"
...
grep -Fq ":revision \"$revision\"" "$baseline_file" \
  || fail "target baseline revision does not match the pinned checkout"
grep -Fq ':baseline-target :rawhttp' "$profile_file" \
  || fail "profile does not select the RawHTTP target baseline"
...
[ -x "$java_home/bin/java" ] && [ -x "$java_home/bin/javac" ] \
  || fail "set DRIPSHARP_JAVA_HOME to a JDK 17 installation"
...
work=$(mktemp -d /tmp/dripsharp-rawhttp-contract.XXXXXX)
cleanup() {
  case "$work" in
    /tmp/dripsharp-rawhttp-contract.*) rm -rf -- "$work" ;;
...
(
  ...
  JAVA_HOME="$java_home" ./gradlew --quiet --console=plain --no-daemon \
    -I "$inventory_script" \
    "${gradle_project}:dripsharpInventoryMain" \
    "-Pdripsharp.project=$gradle_project" \
    "-Pdripsharp.output=$actual_inventory"
)
...
[ "$(sed -n '1p' "$compiled_surface_file")" = "# DRIPSHARP_DOTNET_ACCESSIBLE_CONTRACT_V1" ] \
