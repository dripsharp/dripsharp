The main project we are working on is the repository root (Clojure).

The DripSharp product goal is fixed by
`doc/targets/pkl/product-goal.md`, whose baseline is commit
...
Changes to `doc/targets/pkl/product-goal.md`, the user-approved exclusion list,
...
check available RAM/CPU. Set `DRIPSHARP_WORKERS` to 22, and  and 28 GiB for the JVM heap. Be careful about running workloads that more than an hour. Consider using clj-nrepl-eval on specific parts instead.
