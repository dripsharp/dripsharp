...
(def ^:private signature-probe
  (str
   ...
   "        Assert(JavaCompat.ToHexString(-65536) == \"ffff0000\",\n"
   "               \"Integer.toHexString must preserve 32-bit two's-complement width\");\n"
   "        Assert(JavaCompat.ToHexString(-65536L) == \"ffffffffffff0000\",\n"
   "               \"Long.toHexString must preserve 64-bit two's-complement width\");\n\n"
   "        var xmlSettings = JavaCompat.NewXmlReaderSettings();\n"
   "        JavaCompat.XmlReaderSetNamespaceAware(xmlSettings, false);\n"
   "        var xmlClone = JavaCompat.XmlReaderSettingsClone(xmlSettings);\n"
   "        using var xmlInput = new System.IO.MemoryStream(\n"
   "            System.Text.Encoding.UTF8.GetBytes(\n"
   "                \"<xfdf xmlns='urn:xfdf'><fields><field name='x'/></fields></xfdf>\"));\n"
   "        var xml = JavaCompat.XmlParse(xmlClone, xmlInput);\n"
   "        Assert(xml.DocumentElement?.Name == \"xfdf\" &&\n"
   "               xml.DocumentElement.NamespaceURI == \"\",\n"
   "               \"Namespace-unaware Java DOM parsing must preserve unqualified node names\");\n"
   "        Assert(xml.DocumentElement?.FirstChild?.Name == \"fields\",\n"
   "               \"Namespace-unaware Java DOM parsing must survive settings cloning\");\n\n"
