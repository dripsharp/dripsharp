    ...
    (testing "implementation internals do not become product exclusions"
      (let [internals (filter #(= "public-implementation-internal"
        ...
        (is (every? #(str/starts-with? (:exclusion-evidence %)
                                       "doc/targets/pkl/product-goal.md#User-Approved")
                    exclusions))))
    ...
    (testing "native helper decisions come from authoritative upstream usage"
      (let [decisions contract/native-owner-decisions
            ...
            internal-owners (->> decisions
                                 ...
                                 (map key)
                                 set)]
        (is (= 30 (count decisions)))
        (is (every? #(str/starts-with? (:upstream-provenance %) "research/pkl/")
                    (vals decisions)))
        (is (not-any? #(or (str/includes? (:upstream-provenance %)
                                          "PackageConsumer.cs")
                           (str/includes? (:upstream-provenance %)
                                          "PklCorePackageCorpusRunner.cs"))
                      (vals decisions)))
        (is (= 7 (count command-owners)))
        (is (every? #(= "cli.options" (get-in decisions [% :behavior-family]))
                    command-owners))
...
(deftest package-probe-reflects-generic-nullable-delegate-and-lifecycle-metadata
  (let [project (paths/resolve-path (:root @fixtures) "PackageProbeFixture.csproj")
    ...
    (let [rows (:rows (contract/read-tsv output contract/package-columns))
          ...
          resource (by-owner "DripSharp.PublicApiProbeFixture.Resource`1")
          formatter (by-owner "DripSharp.PublicApiProbeFixture.Formatter`1")]
      (is (seq resource))
...
(deftest public-contract-compiler-preserves-source-only-generic-nullability
  (let [project (paths/resolve-path (:root @fixtures) "PackageProbeFixture.csproj")
        ...
        compiler (paths/resolve-path @workspace "validation"
                                     "public-contract-compiler"
                                     "PublicContractCompiler.csproj")]
    (Files/writeString
     strong-keys
     (str "PackageProbeFixture\t"
          "DripSharp.PublicApiProbeFixture.Resource\tmethod\tMap\t1\n"
          "PackageProbeFixture\t"
          "DripSharp.PublicApiProbeFixture.Resource\tmethod\tMapNullable\t1\n")
     (make-array OpenOption 0))
