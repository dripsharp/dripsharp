...
(defn- temp-directory
  []
  (Files/createTempDirectory "dripsharp-project-test" (make-array FileAttribute 0)))
...
(defn- write-manifest!
  ...
  (let [manifest (paths/resolve-path root "manifest.tsv")
    ...
    (spit (str manifest)
          (str "DRIPSHARP_GRADLE_INPUTS_V3\n"
...
(defn- write-v5-manifest!
  ...
  (let [manifest (paths/resolve-path root "manifest-v5.tsv")]
    (spit (str manifest)
          (str "DRIPSHARP_GRADLE_INPUTS_V5\n"
               (apply str
                      (map #(str (str/join "\t" %) "\n") records))))
    manifest))
...
(deftest gradle-toolchain-failure-is-explicit
  (let [root (temp-directory)
        ...
        _ (create-file! root "gradle/discover-main.gradle")
        error (try
...
(deftest requested-gradle-project-controls-task-and-manifest-identity
  (let [root (temp-directory)
    ...
    (is (some #{":pkl-core:dripsharpDescribeMain"} @seen-command))
    (is (some #{"-Pdripsharp.project=:pkl-core"} @seen-command))))
  ...
  (let [workspace (paths/workspace-root)
        ...
        discovery (project/discover-main!
                   {:workspace-root workspace
                    ...
                    :manifest (paths/resolve-path root "build/dripsharp-inputs.tsv")})
