...
(deftest pkl-package-legal-inputs-and-package-metadata-are-pinned
  (let [workspace (paths/workspace-root)
    ...
    (doseq [[profile-path destination-path]
      ...
      (let [profile (harness/read-profile workspace profile-path)
            destination
            (project-emission/read-configuration workspace destination-path)
            project-text
            ...
            legal-assets
            (filterv #(= "pkl.legal" (namespace (:strategy %))) assets)
            notice-asset
            (first (filter #(= :pkl.legal/notice (:strategy %))
                           legal-assets))
            packaged-notice
        ...
        (testing "NuGet uses the packed license file and packs both legal payloads"
          ...
          (is (not (str/includes? project-text
                                  "PackageLicenseExpression"))))))))

(deftest pkl-package-generation-rejects-changed-legal-inputs
  (let [workspace (paths/workspace-root)]
    (doseq [[profile-path destination-path]
            [["targets/pkl/profiles/parser.edn"
              "targets/pkl/destinations/parser.edn"]
            ...
            (caught
             #((get-in (pkl-project/rule-bundle)
               ...
               {:workspace-root root
                :profile profile
                :configuration destination}))]
        (is (= :pkl-legal-input-mismatch
               (:kind (ex-data error))))
        (is (= (get-in destination [:package :id])
               (:package-id (ex-data error))))
