...
internal static class Program
{
    ...
    private static void VerifyIoAndFontBox()
    {
        ...
        Assert(
            ...
            input.Read() == 3 &&
            input.Read() == -1,
            "DripSharp.PdfCarton.IO random access must preserve unsigned reads and EOF.");
        ...
        Assert(
            ...
            bounds.Contains(3, 4) &&
            !bounds.Contains(7, 4),
            "DripSharp.PdfCarton.Fonts must consume DripSharp.PdfCarton.IO and preserve geometry.");
    }
    ...
    private static byte[] CreateAndRoundTripXmp()
    {
        ...
        dublinCore.SetTitle("x-default", "PdfCarton family metadata");
        ...
        Assert(
            !parsedInput.CanRead,
            "DripSharp.PdfCarton.Xmp parsing must close its consumed input.");
        Assert(
            string.Equals(
                parsed.GetDublinCoreSchema()?.GetTitle("x-default"),
                "PdfCarton family metadata",
                StringComparison.Ordinal),
            "DripSharp.PdfCarton.Xmp metadata must survive serialization and parsing.");
        return bytes;
    }
    ...
    private static string CreateEditAndReopenPdf(string root, byte[] xmp)
    {
        ...
        using (var document = Loader.LoadPDF(new FileInfo(first)))
        {
            Assert(
                ...
                "DripSharp.PdfCarton must reopen the created metadata-bearing PDF.");
        ...
        using (var document = Loader.LoadPDF(new FileInfo(edited)))
        {
            ...
            Assert(
                ...
                metadataText.Contains(
                    "PdfCarton family metadata",
                    StringComparison.Ordinal),
                "DripSharp.PdfCarton edits and XMP metadata must survive reopen.");
        }
        return edited;
    }
        ...
        string pdf)
    {
        ...
        using (var document = (PreflightDocument)parser.Parse())
        {
            ...
            Assert(
                ...
                "DripSharp.PdfCarton.Preflight must validate the ordinary PDF and report PDF/A gaps.");
        }
        Assert(
            source.IsClosed(),
            "DripSharp.PdfCarton.Preflight document disposal must close DripSharp.PdfCarton.IO input.");
        ...
        Assert(
            ...
            "Malformed DripSharp.PdfCarton.IO input must support deterministic cleanup.");
    }
